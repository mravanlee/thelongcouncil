...
function slugify(name) {
  if (!name) return '';
  return name.replace(/\s*\([^)]*\)/g, '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
}
...
function MemberAvatar({ member }) {
  ...
  return (
    <div className="mc-av">
      ...
      {!imgFailed && slug && <img src={`/avatars/avatar_${slug}.webp`} alt="" className="mc-av-img" onError={() => setImgFailed(true)} />}
      <style jsx>{`
...
function PositionText({ text, type }) {
  ...
  return <span><span style={{ fontWeight: 500, color: '#6b1a1a' }}>{text.slice(0, colonIdx)}</span>{text.slice(colonIdx)}</span>;
}
    ...
    <div className="mc-card">
      ...
      <div className="mc-positions">
        {m.positions.map((p, i) => (
          <div key={i} className="mc-pos">
            <span className="mc-pos-dash">—</span>
      ...
      {debates > 0 && (
        <div className="mc-debates">
          <div className="mc-debates-dot" />{debates} debates
        </div>
      )}
      <style jsx>{`
        ...
        .mc-bio { font-family: 'Inter', sans-serif; font-size: 13px; color: #2a2a2a; line-height: 1.65; margin-bottom: 14px; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }
        .mc-divider { border: none; border-top: 0.5px solid #e8e3d8; margin-bottom: 12px; }
        .mc-positions { display: flex; flex-direction: column; gap: 8px; margin-bottom: 12px; flex: 1; }
        .mc-pos { display: flex; gap: 8px; font-family: 'Inter', sans-serif; font-size: 12.5px; color: #2a2a2a; line-height: 1.55; }
        .mc-pos-dash { color: #6b1a1a; flex-shrink: 0; }
        .mc-core { background: #f0ede3; border-radius: 2px; padding: 10px 13px; margin-bottom: 12px; min-height: 110px; }
        ...
        .mc-debates { font-family: 'Inter', sans-serif; font-size: 11px; color: #9a9a9a; display: flex; align-items: center; gap: 5px; }
...
// ── Council roster — 37 members, alphabetical ────────────────────────────
const COUNCIL_MEMBERS = [

  { type: 'thinker', monogram: 'AH', name: 'Albert Hirschman', role: 'Economist · Exit, Voice and Loyalty', lifespan: '* 1915 · † 2012', country: 'Germany / USA', bio: 'The ultimate optimist in a field full of gloom. Fled the Nazis, fought in Spain, reinvented himself in Latin America. Concluded that disorder is not an obstacle to development but its engine.', positions: ['Unbalanced growth: progress comes from tension between sectors, not from everything working smoothly at once.', 'The hiding hand: we underestimate both the difficulties ahead and our ability to overcome them. That gap is what makes progress possible.', 'Exit, voice, loyalty: when an institution fails, people leave, complain, or stay silent. Which option is available determines whether it improves.'], corebelief: 'Stop trying to control everything. The creativity that solves the problem emerges from the chaos of attempting it.' },

  { type: 'thinker', monogram: 'AA', name: 'Ali ibn Abi Talib', role: 'Fourth Caliph of Islam · Letter to Malik al-Ashtar', lifespan: '* 601 · † 661', country: 'Arabia', bio: 'Cousin and son-in-law of the Prophet, fourth Caliph of Islam. His letter to the governor of Egypt remains one of history\'s most sophisticated texts on the obligations of power. Assassinated in 661. His death split Islam into Sunni and Shia.', positions: ['Justice before stability: a ruler who maintains order through injustice has not solved the problem but postponed it.', 'Fair taxation: tax according to what the land produces, not according to what you need. Greed destroys the prosperity it tries to extract.', 'Judicial independence: if the judge fears the ruler, there is no justice. The test of governance is whether the powerful can be held to account.'], corebelief: 'The ruler who governs for himself will lose everything. The ruler who governs for his people might keep it.' },

  { type: 'thinker', monogram: 'AS', name: 'Amartya Sen', role: 'Economist · Development as Freedom', lifespan: '* 1933', country: 'India / UK', bio: 'Watched people starve in Bengal as a child, not because there was no food but because they had no money to buy it. Spent his life proving that poverty is a political failure, not a natural condition. No functioning democracy with a free press has ever had a famine.', positions: ['The capability approach: development means expanding what people can actually do and be. GDP misses most of what matters.', 'Famines and democracy: famines happen when governments have no incentive to prevent them. A free press and elections fix that incentive fast.', 'Women\'s education: educate women and everything else improves. It is the highest-return investment in development.'], corebelief: 'Poverty is not a natural condition. It is a political failure — and political failures can be corrected.' },

  { type: 'thinker', monogram: '孔', name: 'Confucius', role: 'Philosopher · The Analects', lifespan: '* 551 BC · † 479 BC', country: 'China', bio: 'Spent decades trying to persuade rulers to govern morally. None of them listened. He died believing he had failed. Two and a half thousand years later his ideas still shape how a third of the world thinks about authority, merit and obligation.', positions: ['Rectification of names: when rulers call things what they are not, governance becomes confused. Clarity of language is the foundation of order.', 'Meritocracy: authority should come from ability and character, not birth or wealth. Reward the wrong people and you will be governed by them.', 'The ruler\'s obligation: the people\'s trust is the only thing a ruler cannot afford to lose. Armies and wealth can be rebuilt. Trust cannot.'], corebelief: 'Govern yourself before you govern others. Moral authority is not a luxury — it is the only authority that lasts.' },

  { type: 'leader', monogram: 'BG', name: 'David Ben-Gurion', role: 'Prime Minister, Israel 1948–53; 1955–63', lifespan: '* 1886 · † 1973', country: 'Israel', bio: 'Declared Israeli independence in 1948 with five Arab armies already massing on the border. Simultaneously built a state and fought a war, then resigned twice and came back twice. Understood that survival is not a policy choice but the precondition of all others.', positions: ['Security first: a state without defensible borders cannot govern, plan or promise its citizens anything. Everything else comes after.', 'Build while fighting: institution-building cannot wait for peace. If you wait, there will be no state when peace arrives.', 'Pragmatic alliances: align with whoever keeps you alive. Ideology is a luxury small states cannot afford.'], corebelief: 'Survival is not a policy choice. It is the precondition of all others. A state that forgets this will not last long enough to remember.' },
  ...
  { type: 'thinker', monogram: 'EO', name: 'Elinor Ostrom', role: 'Economist · Governing the Commons', lifespan: '* 1933 · † 2012', country: 'United States', bio: 'Spent her career studying fishing villages, forests and irrigation canals to test whether communities can govern shared resources without privatisation or state control. They can, and often better. First woman to win the Nobel Prize in Economics, in 2009.', positions: ['The commons: fishing villages, forests and canals can be governed collectively by the communities that depend on them — often better than markets or states.', 'Polycentric governance: no single structure fits all situations. Standardising them destroys what makes each work.', 'Local knowledge: the farmer who worked the same land for thirty years knows things no central planner ever will.'], corebelief: 'Neither the market nor the state has a monopoly on good governance. Communities can solve what both fail to fix.' },
  ...
  { type: 'thinker', monogram: 'NMa', name: 'Niccolo Machiavelli', role: 'Statesman · The Prince', lifespan: '* 1469 · † 1527', country: 'Florence', bio: 'Served the Florentine Republic loyally for 14 years, was tortured after the Medici returned to power, and retired to write the books that made him famous. The Prince was a job application that never worked. He died without the career he spent his life trying to restore.', positions: ['Effective power over moral power: the question is not whether force is just — it is whether it is effective. A ruler who is only good will be destroyed.', 'Strategic patience: timing is everything. Strike at the right moment, not the righteous one.', 'Fear over love: it is better to be feared than loved when you cannot be both. But it is fatal to be hated.'], corebelief: 'Everyone sees what you appear to be. Few experience what you really are. In politics, appearances are not a distraction from reality — they are part of it.' },

  { type: 'leader', monogram: 'OP', name: 'Olof Palme', role: 'Prime Minister, Sweden 1969–76; 1982–86', lifespan: '* 1927 · † 1986', country: 'Sweden', bio: 'Led Sweden to take clear positions on Vietnam, apartheid and nuclear weapons when it would have been easier to stay quiet. Showed that a small country with no military power could still matter in world politics, if it was consistent enough.', positions: ['Security is not zero-sum: building safety at your neighbour\'s expense creates the next conflict. The adversary must also feel secure.', 'A small state\'s only currency is consistency: one exception and the credibility is gone. There is no rebuilding it.', 'Equality and efficiency reinforce each other: the trade-off is smaller than its opponents need it to be.'], corebelief: 'A small country cannot project military power. It can project moral consistency. That turns out to be worth more.' },
  ...
  { type: 'leader', monogram: 'SB', name: 'Simón Bolívar', role: 'President, Gran Colombia 1819–30', lifespan: '* 1783 · † 1830', country: 'Venezuela', bio: 'Liberated six countries from Spanish rule, then watched them fragment. Died in 1830 convinced he had ploughed the sea. The United States of South America he dreamed of never came. What came instead were the caudillos he had always feared.', positions: ['Liberation is the beginning, not the end: the skills that win independence are not the skills that build a state.', 'Strong executive or fragmentation: post-colonial republics need a strong centre or they dissolve into regional strongmen.', 'Regional unity or external domination: history offers no stable third option for small states in a world of great powers.'], corebelief: 'Independence without unity is just a change of masters. Building a state that holds together is harder than winning the war and more important.' },

  { type: 'thinker', monogram: 'ST', name: 'Sun Tzu', role: 'Strategist · The Art of War', lifespan: '* 544 BC · † 496 BC', country: 'China', bio: 'May or may not have existed as a single person. The Art of War almost certainly did not come from one hand. None of that has mattered. Fifteen centuries of generals and every business school in the world have found it indispensable.', positions: ['Supreme strategy: the highest form of strategy is to defeat the adversary without fighting. Force is expensive, slow and produces enemies.', 'Know yourself and your enemy: the commander who does not know his own weaknesses loses every battle, however large his army.', 'Deception as strategy: appear weak when strong, strong when weak. The adversary who misreads you has already lost.'], corebelief: 'The general who wins the battle has already won it in his mind before it is fought. The general who loses has fought first and sought victory afterwards.' },
