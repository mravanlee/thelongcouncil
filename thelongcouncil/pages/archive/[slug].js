...
export default async function handler(req) {
  try {
    ...
    // Parallel fetch: session + 4 fonts in one round-trip instead of sequential.
    // Cuts cold-render latency roughly in half — important for share-fetchers
    // (WhatsApp/iMessage) which time out aggressively on first crawl.
    const [sessionRes, playfair500, playfair600, playfairItalic, inter500] = await Promise.all([
