...
- `scripts/refresh-related.py` — recomputes the "Related Debates" (`sessions.related`) for every session using local BGE embeddings (no API key). Needs the venv at `~/.cache/tlc-related/venv` (has `fastembed`). Dry run: `~/.cache/tlc-related/venv/bin/python scripts/refresh-related.py`; write: add `--write`. Re-run after adding debates.
...
- **`related`** (jsonb, nullable) — precomputed "Related Debates" for the detail page: `[{slug, title, blurb}]`. Filled OFFLINE by `scripts/refresh-related.py` (local BGE embeddings + structured re-rank); the live site only reads it. Not part of the Vercel/Supabase serving path, so new debates only get related links after the next refresh run.
