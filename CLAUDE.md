...
- **The site is intentionally English-only. Do NOT reintroduce language mirroring.** Visitors may type a question in any language, but everything shown is English. The pipeline's first prompt translates every non-English question into English (`pages/api/pipeline.js`, the `LANGUAGE:` step) so the debate/verdict/brief are always English. The **sharpener** (`pages/api/sharpen.js`) also replies only in English: it translates a non-English question into an English READY line, and its CLARIFY/DECLINE copy is English. (It used to mirror the user's language — "if they write in Dutch, respond in Dutch" — which left a Dutch visitor with a Dutch sharpener step but an English debate. Removed Jun 25 2026.) The out-of-scope **DECLINE** path uses one fixed English message (`DECLINE_MESSAGE` constant) — the model only flags scope, it never writes the copy, so the wording never drifts.
